...
import { vueRelativeScopeMarkerName } from '../vueScopedPolicy'
import type {
  AttributeSelector,
  CombinatorSelector,
...
export function isDescendantCombinator(
  ...
  return component?.type === 'combinator' && component.value === 'descendant'
}
...
export function isRelativeScopeMarker(component: SelectorComponent): boolean {
  return (
    component.type === 'pseudo-class' &&
    component.kind === 'where' &&
    component.selectors.length === 1 &&
    component.selectors[0].length === 1 &&
    component.selectors[0][0].type === 'attribute' &&
    component.selectors[0][0].name === vueRelativeScopeMarkerName
  )
}
