...
export function createStyleLightningCSSVisitor(
  ...
  const hasAnimationDeclarations = features?.hasAnimationDeclarations ?? false
  const hasScopedSelectorSpecials = features?.hasScopedSelectorSpecials ?? true
  const hasVBind = features?.hasVBind ?? true
  const keyframes = features?.keyframes
